...
spatial_data_fusion_engine_CV <- function(pr_xyz,
                                          pr_data,
                                          features_grid,
                                          params_mod = list(
                                            Nstations = 60,
                                            ...
                                            ))
{
  ...
  PredObs_df <- parallel::mclapply(
    ...
    function(jxi) {
      ...
      nearby_ID <- PrObs_df[match(nearby_ID$out, PrObs_df$ID),]
      nearby_ID <- nearby_ID
      ...
      data.frame(obs = nearby_ID$PrObs[1], mod = response[1])
