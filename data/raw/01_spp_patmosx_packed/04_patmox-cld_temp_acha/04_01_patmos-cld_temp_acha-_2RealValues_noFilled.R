# cld_temp_acha	K	-32767*	32767*	0.00244148	240	meters	None Cloud-top temperature computed using ACHA (CDR variable) 
rm(list = ls())
...
for(year_x in years_data){
  ...
  path_data <- paste("/mnt/climstor2/vol01_ecmwf/download/patmosx/cld_temp_acha/raw/", year_x, sep = "")
  ...
  main_tp = "/scratch2/ahuerta/patmosx/cld_temp_acha/raw_rv"
  ...
  lapply(seq_along(names(r_stack)),
         function(ijx){
           ...
           placeholder <- (placeholder * 0.00244148) + 240 - 273.15  # SCALING + Offset (from K -> °C)
