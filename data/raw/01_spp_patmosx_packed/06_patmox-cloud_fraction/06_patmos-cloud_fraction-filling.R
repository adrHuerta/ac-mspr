# it was noticed that for cloud_fraction, there were three dates that were empty: 2019-11-19 / 2019-08-05 / 2019-08)
...
raster_files <- list.files("/mnt/climstor2/vol01_ecmwf/download/patmosx/cloud_fraction/raw/2019", pattern = "\\.tif$", full.names = TRUE)
...
for (date_str in dates_missing) {
  out_path <- file.path("output_2019_cloud_fraction/", paste(date_str, ".tif", sep = ""))
...
# cp -a output_2019_cloud_fraction/. /mnt/climstor2/vol01_ecmwf/download/patmosx/cloud_fraction/raw/2019/
...
plot(filled_r[[c( dates_with_full_NA_index - c(2,1), dates_with_full_NA_index, dates_with_full_NA_index + 1:2)]] * 0.00393701 +	0.5)
plot(r_stack[[c( dates_with_full_NA_index - c(2,1), dates_with_full_NA_index, dates_with_full_NA_index + 1:2)]] * 0.00393701 +	0.5)
