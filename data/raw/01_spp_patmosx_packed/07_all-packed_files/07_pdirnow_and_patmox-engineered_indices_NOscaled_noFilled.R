...
main_path_to_read <- "/scratch2/ahuerta/patmosx"
main_gsmap_path_to_read <- "/mnt/climstor2/vol01_ecmwf/download/pdirnow/"
main_path_to_save <- file.path("/mnt/climstor2/vol01_ecmwf/download/patmosx", "features_pdirnow")
...
for(year_x in 2000:2021){
  
  if(year_x == 2000) {
    
    year_x_date <- seq(as.Date(paste0(year_x, "-", "03-01")), as.Date(paste0(year_x, "-", "12-31")), freq = "day")
  ...
  lapply(
    ...
    function(year_x_date_i){
      ...
      pr_sat_data <- file.path(main_gsmap_path_to_read, "raw4", year_x, year_x_date_i)
      ...
      PrSat <- resample(PrSat, CF_gra_n)
      crs(PrSat) <- crs(CF_gra_n)
      PrSat_bin <- resample(PrSat_bin, CF_gra_n)
      crs(PrSat_bin) <- crs(CF_gra_n)
