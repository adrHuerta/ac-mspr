# it was noticed that for cld_press_acha, there was one date that was empty: 2019-11-19
...
raster_files <- list.files("/mnt/climstor2/vol01_ecmwf/download/patmosx/cld_press_acha/raw/2019", pattern = "\\.tif$", full.names = TRUE)
...
for (date_str in dates_missing) {
  out_path <- file.path("output_2019_cld_press_acha/", paste(date_str, ".tif", sep = ""))
...
# cp -a output_2019_cld_press_acha/. /mnt/climstor2/vol01_ecmwf/download/patmosx/cld_press_acha/raw/2019/
...
plot(filled_r[[c( dates_with_full_NA_index - c(2,1), dates_with_full_NA_index, dates_with_full_NA_index + 1:2)]] * 0.0167852 +	550)
plot(r_stack[[c( dates_with_full_NA_index - c(2,1), dates_with_full_NA_index, dates_with_full_NA_index + 1:2)]] * 0.0167852 +	550)
