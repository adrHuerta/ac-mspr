# it was noticed that for cloud prob, there were three dates that were empty: 2019-11-19 / 2019-08-05 / 2019-08)
...
raster_files <- list.files("/mnt/climstor2/vol01_ecmwf/download/patmosx/cloud_probability/raw/2019", pattern = "\\.tif$", full.names = TRUE)
...
template_raster <- raster(template_file)

# ---- Step 5: Create placeholder rasters for missing dates ----
for (date_str in dates_missing) {
  out_path <- file.path("output_2019_cloud_prob/", paste(date_str, ".tif", sep = ""))
  placeholder <- setValues(template_raster, NA) 
  writeRaster(placeholder, out_path, format = "GTiff", overwrite = TRUE)
  cat("Created placeholder:", out_path, "\n")
}

# Now within climcal3 move the files
# cp -a output_2019_cloud_prob/. /mnt/climstor2/vol01_ecmwf/download/patmosx/cloud_probability/raw/2019/
...
plot(filled_r[[1]] - r_stack[[1]])

dates_with_full_NA <- c("2019-08-05", "2019-08-06", "2019-11-19")
dates_with_full_NA_index <- match(dates_with_full_NA, names(r_stack))

plot(filled_r[[c( dates_with_full_NA_index[1] - 1:2, dates_with_full_NA_index[1:2], dates_with_full_NA_index[2] + 1:2)]])
plot(r_stack[[c( dates_with_full_NA_index[1] - 1:2, dates_with_full_NA_index[1:2], dates_with_full_NA_index[2] + 1:2)]])

plot(filled_r[[c( dates_with_full_NA_index[1] - 1:2, dates_with_full_NA_index[1:2], dates_with_full_NA_index[2] + 1:2)]] - 
       r_stack[[c( dates_with_full_NA_index[1] - 1:2, dates_with_full_NA_index[1:2], dates_with_full_NA_index[2] + 1:2)]])

plot(filled_r[[c( dates_with_full_NA_index[3] - 1, dates_with_full_NA_index[3], dates_with_full_NA_index[3] + 1)]])
plot(r_stack[[c( dates_with_full_NA_index[3] - 1, dates_with_full_NA_index[3], dates_with_full_NA_index[3] + 1)]])

plot(filled_r[[c( dates_with_full_NA_index[3] - 1, dates_with_full_NA_index[3], dates_with_full_NA_index[3] + 1)]] - 
       r_stack[[c( dates_with_full_NA_index[3] - 1, dates_with_full_NA_index[3], dates_with_full_NA_index[3] + 1)]])
