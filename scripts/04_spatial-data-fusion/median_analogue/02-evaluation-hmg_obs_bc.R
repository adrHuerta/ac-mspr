...
for(date_to_merge in cv_days$dates){
  ...
  get_analogues_dates <- readRDS(paste0("output/03_precipitation-pattern-analogue/cv/cv-hmg_obs_bc/",
                                         date_to_merge, ".RDS"))
  get_analogues_dates <- transform(get_analogues_dates, mean_metric = (dr  + dr_p90 + mcc)/3)
  get_analogues_dates <- get_analogues_dates[order(get_analogues_dates$mean_metric, decreasing = TRUE), ][trunc(nrow(get_analogues_dates)/2), ]
  ...
  saveRDS(
    output_cv,
    file = file.path("output/04_spatial-data-fusion/cv_median/cv-hmg_obs_bc",
