...
for(date_to_merge in cv_days$dates){
  ...
  get_analogues_dates <- get_analogues_dates[order(get_analogues_dates$mean_metric, decreasing = TRUE), ][1, ]
  ...
  saveRDS(
    output_cv,
    file = file.path("output/04_spatial-data-fusion/cv_NGF_features/cv-hmg_obs_bc",
