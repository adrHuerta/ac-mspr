...
exp_plot_label <- exp_plot[exp_plot$anlg %in% c("09", "13", "41"),]
...
pr_new_df <- pr_new_s[[which(exp_plot$anlg %in% c("09", "13", "41"))]]
pr_new_df <- c(pr_new_df, app(pr_new_s, median), app(pr_new_s, sd))
names(pr_new_df) <- c("analogue-09", "analogue-13", "analogue-41", "analogue-median", "analogue-sd")

NoNAss <- pr_data_r[,!is.na(unlist(pr_data_r))] # It has NAs!
pr_new_df_metric <- fast_dr_mcc_v(obs = matrix(NoNAss, ncol = 1), mod_mat = extract(pr_new_df, pr_xyz_r[match(names(NoNAss), pr_xyz_r$ID)]))
...
ggsave(
  "output/01_example-ac-mspr/exp-qc_obs.pdf",
  units = "in",
  width = 6,
  height = 7,
  dpi = 300,
